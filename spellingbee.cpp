...
/* Solve the Spelling Bee game.
   Parameters:
      dictFileName: the name of the file containing the list of words
      letters:      a string of 7 letters, with the middle letter first
      score:        the score assigned to the words found
   Return value: the number of valid words given the string of letters       */
   ...
   int spellingBee(string dictFileName, string letters, int& score)
   {
     ...
     while(getline(myFile, word)) //Checks to see if the file is good
     {
       //------//
       if(checkWord(word, letters) == SB_WORD && word.length() == 4)
       {
         wordCount++;
         score = score + 1;
       }
